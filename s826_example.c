...
#define DIOSTATE(STATES,CHAN)   ((STATES[CHAN / 24] >> (CHAN % 24)) & 1)    //but it makes the code easier to read and understand. In a real
...
int main(int argc, char **argv)
{
    ...
    if (boardflags < 0)
        ...
        printf("creating pwm");
        CreatePWM(0, 0, 900, 500);
        RouteCounterOutput(0, 0, 0);
        StartPWM(0, 0);
        uint t1, t0 = CurrentTimestamp(0);

        while (1) {
            t1 = CurrentTimestamp(0);
            if ( t1 - t0 > 1476500) break;
        }
